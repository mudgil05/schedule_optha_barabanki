...
module.exports = async function handler(req, res) {
  ...
  const MODEL = 'gemini-2.0-flash';  // updated from deprecated gemini-1.5-flash
...
{"rows":[
  ...
  try {
    const geminiRes = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/${MODEL}:generateContent`,
      {
        ...
        body: JSON.stringify({
          ...
          generationConfig: { temperature: 0, maxOutputTokens: 8192 }  // increased from 4096
        })
      }
    );
    ...
    // Check if Gemini stopped early due to token limit
    const finishReason = data.candidates?.[0]?.finishReason;
    if (finishReason === 'MAX_TOKENS') {
      return res.status(200).json({ rows: [], warning: 'Response was cut off — PDF page may be too complex. Try splitting into smaller pages.' });
    }
